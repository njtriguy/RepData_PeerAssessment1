...
```{r average numbers of steps per interval}
...
plot(interval_mean$interval,interval_mean$steps,type="p",xlab="5 Min Time Intervals",ylab="Average Number Of Steps",
     main="Average Steps Vs. 5 Minute Time Interval")

# calculate the smooth average across all 5 minute step intervals
smooth_avg<-smooth.spline(interval_mean$interval,interval_mean$steps, df=30)  # calculate smoothing average across all intervals
...
The maximum average number of steps is `r max_avg_value` and is found at time interval `r interval_max_avg`.
...
DF_groups <- merge(DF,aggregate(steps ~ interval, data=DF,mean),by="interval",suffix=c("",".mean"))  # calculate the interval group means
DF_groups <- DF_groups %>%  mutate(steps = ifelse(is.na(steps),steps.mean,steps))  # step the value to the mean where step value is NA
DF_groups <- DF_groups[,c(1,2,3)]  # remove the mean column
